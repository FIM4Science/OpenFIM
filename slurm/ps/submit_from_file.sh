...
# run_in_fim_venv.sh must be exectutable and in a container
...
while IFS= read -r line <&3; do
	...
	eval "srun ${srun_params} --job-name=${filename_start}_${itr} $HOME/slurm_scripts/ps/run_in_fim_venv.sh '${line}'" # &
	((itr += 1))
done
