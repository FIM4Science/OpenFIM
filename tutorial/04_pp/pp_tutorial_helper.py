...
def plot_intensity_comparison(model_output: dict, batch: dict, path_idx: int = 0):
    ...
    event_times = inference_event_times[0, path_idx, :seq_len, 0].numpy()
    event_types = inference_event_types[0, path_idx, :seq_len, 0].numpy()
    ...
    for mark_idx, axis in enumerate(axes):
        ...
        axis.plot(valid_eval.numpy(), pred_curve[sort_idx].numpy(), color="#0072B2", linewidth=2.0, label="FIM-PP")

        is_current_mark = event_types == mark_idx

        # Calculate Y-coordinates for all events using interpolation
        x_vals = valid_eval.numpy()
        y_vals = pred_curve[sort_idx].numpy()
        all_event_y = np.interp(event_times, x_vals, y_vals)

        # Plot OTHER marks first
        if np.any(~is_current_mark):
            axis.scatter(
                event_times[~is_current_mark],
                all_event_y[~is_current_mark],
                color="#A0A0A0",
                s=10,
                alpha=1,
                label="Event Other Mark",
                zorder=10,
            )

        # Plot CURRENT mark on top
        if np.any(is_current_mark):
            axis.scatter(
                event_times[is_current_mark], all_event_y[is_current_mark], color="#CC79A7", s=40, label="Event Current Mark", zorder=10
            )

        axis.set_ylabel(f"Intensity of Mark {mark_idx}")
