...
- FIMs for Zero-shot Time Series Imputation (@fim_imputation, [huggingface](https://huggingface.co/FIM4Science/fim-imputation))

```{note}
Take a look at our [tutorials](01_introduction/tutorials.md)!
```
...
To build such FIMs, we follow a general three-step pretraining strategy ({ref}`fim_workflows`, upper right):

```{figure} imgs/fim_workflows.png
---
name: fim_workflows
---
FIM workflows
...
- FIMs can accurately infer the governing dynamical system in zero-shot (in-context) mode from unseen, noisy, and heterogeneous datasets ({ref}`fim_workflows`, lower left); and
- FIMs can also rapidly be finetuned to target datasets ({ref}`fim_workflows`, lower right).
