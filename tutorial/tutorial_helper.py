...
def visualize_prediction(ts, ys_true, ys_noisy, prediction, mask):
    ...
    if ts.shape == prediction.shape:
        plt.scatter(ts[mask], prediction[mask], label="Predicted values", c="black", marker="x")
    else:
        plt.scatter(ts[mask], prediction, label="Predicted values", c="black", marker="x")
    plt.xlabel("t")
    plt.ylabel("x")
    plt.legend()
    plt.show()
...
def prepare_data_sliding_windows(ts, values, mask, num_windows=32, overlap_size=5):
    ...
    stacked_batch = {
        key: torch.cat([b[key] for b in batches], dim=0) for key in batches[0].keys() if key not in ["window_index", "window_range"]
    }
...
def visualize_sde_prediction(trajectories, times, locations, drift, diffusion, plot="duffing"):
    ...
    for i in range(min(5, trajectories.shape[0])):
        if trajectories.shape[2] == 1:  # 1D system
            ...
            dx_vals = (trajectories[i, 1:, 0] - trajectories[i, :-1, 0]) / (times[i, 1:] - times[i, :-1] + 1e-8)
