...
class SparseGPSDE:
    ...
    def get_inducing_points(self, num_inducing_points, realization_as_inducing=False):
        if not realization_as_inducing:
            ...
            ranges_ = define_grid_ranges(self.dense_path_realization, ignore_percentatge=0.0)
            inducing_points = define_mesh_points(total_points=self.num_inducing_points, n_dims=self.dimensions, ranges=ranges_)
        else:
            inducing_points = self.dense_path_realization[:-1, :]
            self.num_inducing_points = self.dense_path_realization.size(0) - 1
        return inducing_points

    def inference(self, diffusion):
        """ """
