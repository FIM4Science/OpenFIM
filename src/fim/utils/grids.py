...
def random_size_consecutive_locations(
    ...
    if size_distribution == 'poisson':
        av_num_observations = observation_time_params.get("av_num_observations",int(.8*T))
        num_observed = torch.poisson(torch.full((B, P), float(av_num_observations))).clamp(1, T).int()  # Poisson sampling, limited to [1, T]
    ...
    selection_mask = (range_tensor >= start_indices.unsqueeze(-1)) & \
                    (range_tensor < (start_indices + num_observed).unsqueeze(-1))  # mask of selected indices
