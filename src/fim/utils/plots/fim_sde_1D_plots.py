...
model_data_pickle_path = Path("evaluations/synthetic_datasets/01221033_30k_deg_3_ablation_studies_only_learn_scale/model_evaluations/11M_params/wang_double_well_25000_points/default11M_params_wang_double_well_25000_points.pickle")
ground_truth_data_folder_path = Path("/cephfs_projects/foundation_models/data/SDE/test/20241223_opper_and_wang_cut_to_128_lenght_paths/double_well_wang_25000_points")
comparison_model_data_folder_path = Path("data/processed/test/20250117_wang_estimated_equations/bisde_est_double_well_25000_points_split_128_length")
...
ground_truth_values, ground_truth_drift, ground_truth_diffusion = ground_truth_data["locations"], ground_truth_data["drift_at_locations"], ground_truth_data["diffusion_at_locations"]
comparison_model_values, comparison_model_drift, comparison_model_diffusion = comparison_model_data["locations"], comparison_model_data["drift_at_locations"], comparison_model_data["diffusion_at_locations"]
...
ground_truth_values, ground_truth_drift, ground_truth_diffusion = ground_truth_values[:, ::n], ground_truth_drift[:, ::n], ground_truth_diffusion[:, ::n]
comparison_model_values, comparison_model_drift, comparison_model_diffusion = comparison_model_values[:, ::n], comparison_model_drift[:, ::n], comparison_model_diffusion[:, ::n]
...
def create_1D_plot(
    ...
    zoom_areas_drift: list = [dict(xlim=(-4, -3.8), ylim=(50, 60))],  # List of dicts with 'xlim' and 'ylim' for each plot
    zoom_areas_diffusion: list = [dict(xlim=(-4, -3.5), ylim=(3.7, 4.5))],  # Same as above for diffusion
    **kwargs,
):
    ncols = 2
...
def plot_1D_axes(
    ...
    **kwargs
    ...
    if zoom_area_drift:
        ...
        mark_inset(ax_drift, inset_ax_drift, loc1=zoom_area_drift.get('mark_loc1', 2), loc2=zoom_area_drift.get('mark_loc2', 4), fc="none", ec="0.5")

    if zoom_area_diffusion:
        inset_ax_diffusion = inset_axes(ax_diffusion, width="30%", height="30%", loc=zoom_area_diffusion.get('loc', 'lower left'), borderpad=2)
        ...
        mark_inset(ax_diffusion, inset_ax_diffusion, loc1=zoom_area_diffusion.get('mark_loc1', 2), loc2=zoom_area_diffusion.get('mark_loc2', 4), fc="none", ec="0.5")
