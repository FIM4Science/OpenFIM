...
from fim.data_generation.sde.dynamical_systems_sample import get_dynamicals_system_data_from_yaml
...
class SimpleODEDataset(torch.utils.data.Dataset):
    """
    Dataset wrapper around one or more .pt files produced by
    scripts/ode/generate_dummy_ode_dataset.py.

    Each file must be a dict whose values are tensors with the same size along
    dim-0 (the sample dimension S). Multiple files are concatenated along dim-0
    so that different systems (VDP, FHN, Lorenz) can be mixed in one split.

    When files with different state-space dimensionalities are combined (e.g.
    VDP/FHN at D=2 and Lorenz at D=3), pass ``max_dim`` to zero-pad all tensors
    whose last axis is the state dimension up to that common size.

    Expected keys (all optional except obs_values / obs_times / obs_mask):
        obs_values             [S, T, N, D]
        obs_times              [S, T, N, 1]
        obs_mask               [S, T, N, 1]
        locations              [S, L, D]
        drift_at_locations     [S, L, D]
        drift_at_observations  [S, T, N, D]
        dimension_mask         [S, 1, D]
    """
    ...
    def __init__(
        ...
        for p in paths:
            ...
            if max_dim is not None:
                data = self._pad_state_dim(data, max_dim)
            for k, v in data.items():
                chunks.setdefault(k, []).append(v)

        self.data = {k: torch.cat(vs, dim=0) for k, vs in chunks.items()}
        sizes = {k: v.shape[0] for k, v in self.data.items()}
        assert len(set(sizes.values())) == 1, f"Sample-dim mismatch across keys: {sizes}"
        self._len = next(iter(sizes.values()))

    @classmethod
    def _pad_state_dim(cls, data: dict, max_dim: int) -> dict:
        """Zero-pad the last axis (state dim D) of all _DIM_KEYS tensors to max_dim."""
        for key in cls._DIM_KEYS:
            if key not in data:
                continue
            t = data[key]
            d = t.shape[-1]
...
class SimpleODEDataLoader(BaseDataLoader):
    """
    DataLoader for FIMODE that reads flat .pt files generated by
    scripts/ode/generate_dummy_ode_dataset.py.

    Parameters
    ----------
    paths : dict
        Mapping from split name ("train", "validation", "test") to a path or
        list of paths.
    batch_size : int
        Training batch size.
    test_batch_size : int
        Batch size for validation and test splits.
    num_workers : int
        Passed to torch DataLoader (default 0 — single process).
    max_dim : int or None
        If set, zero-pad the state dimension of all loaded tensors to this size.
    """
    ...
    def __init__(
        ...
        for split, split_paths in paths.items():
            self.dataset[split] = SimpleODEDataset(split_paths, max_dim=max_dim)

        self._init_dataloaders(self.dataset)

    def _init_dataloaders(self, dataset: dict):
        for split, ds in dataset.items():
            ...
            bs = self.batch_size if split == "train" else self.test_batch_size
            self.iter[split] = DataLoader(
                ds,
                batch_size=bs,
...
DataLoaderFactory.register("SimpleODEDataLoader", SimpleODEDataLoader)
