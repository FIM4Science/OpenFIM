...
from helpers import eval_ref_fhn, preprocess_ref_fhn
...
_ft_spec = importlib.util.spec_from_file_location("fim_ode_ft", _ROOT / "scripts" / "ode" / "finetune.py")
...
def find_finetuned_checkpoints(task_prefix: str) -> List[Tuple[str, Path]]:
    ...
    results = [(p.parent.parent.name, p) for p in sorted(RESULTS_DIR.glob(f"{task_prefix}*/checkpoints/best.pt"), reverse=True)]
    results += [(p.parent.parent.name, p) for p in sorted(RESULTS_DIR.glob(f"{task_prefix}*/checkpoints/*_best.pt"), reverse=True)]
    return results
...
def main():
    ...
    parser.add_argument("--pretrained", default=str(PRETRAINED), help="Local checkpoint dir with config.json + model.safetensors")
    ...
    print(f"{'=' * 60}")
    print("  Task: fhn  [segmented context — paper / notebook protocol]")
    print(f"{'=' * 60}")
    print("  Context: 2 padded segments with boolean mask (no gap-crossing Δx/Δt).")
    print("  Honest zero-shot evaluation as reported in the paper (~0.40).")
    print("  Finetuned models skipped here — they were trained on flat context.\n")
    print(header)
    print(sep)
    ...
    print(f"{'=' * 60}")
    print("  Task: fhn  [flat 38-point context — finetuning protocol]")
    print(f"{'=' * 60}")
    print("  Context: all 38 observed points as one trajectory (includes both sides")
    print("  of the gap).  Lower zero-shot MSE (~0.21) — post-gap context leaks in.")
    print("  Correct protocol for finetuned models since training used this format.\n")
    print(header)
    print(sep)
