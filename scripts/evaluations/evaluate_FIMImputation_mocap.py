...
def prepare_data(data_dir: str, window_count: int = 3, max_length_window: int = 128):
    """
    ...
    for sample_id in range(obs_grid.size(0)):
        ...
        if padding_size > 0:
            loc = torch.cat([loc, loc[-1].repeat(padding_size)])
...
if __name__ == "__main__":
    ...
    output_dir_base = f"reports/FIMImputation/MocapData/{model_abbr}/"
