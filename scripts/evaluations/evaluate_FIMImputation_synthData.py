...
def evaluate_one_configuration(model_checkpoint_path: str, dl: TimeSeriesDataLoaderTorch, output_base_dir: str):
    ...
    predictions = {}
    loader = dl.test_it if dl.test_it is not None else dl.train_it
    for id, batch in tqdm(enumerate(loader), desc=f"Evaluating {model_abbr}"):
        if id == 0:
