...
def load_system_results(all_results: list[dict], system_name: str, tau: float, exp: int, apply_sqrt_to_diffusion: bool):
    ...
    Args:
        ...
        (system_name, tau, exp): defines the results to extract.
...
if __name__ == "__main__":
    ...
    experiment_to_plot = 0  # we repeat each experiment 5 times

    # choose visual best experiment / tau per model
    select_gp_results = {
        "Double Well": {"tau": 0.002, "exp": 1},
        "Wang": {"tau": 0.002, "exp": 0},
        "Damped Linear": {"tau": 0.002, "exp": 1},
        "Damped Cubic": {"tau": 0.002, "exp": 0},
        "Duffing": {"tau": 0.002, "exp": 0},
        "Glycosis": {"tau": 0.01, "exp": 3},
        "Hopf": {"tau": 0.002, "exp": 0},
    }
    ...
    models_to_plot = [bisde_plot_config, fimsde_plot_config]
    ...
    for system_num, system in enumerate(systems_to_plot):
        axs[system_num, 0].set_ylabel(system, fontsize=5)
    ...
    bbox_y = axs[0, 1].get_position().y1 * 1.12
