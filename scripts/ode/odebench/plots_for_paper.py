...
if False:
    ...
    def plot_vector_field(
        ...
        if use_streamplot:
            ...
            sm = ScalarMappable(norm=Normalize(vmin=magnitude.min(), vmax=magnitude.max()), cmap="viridis")
            sm.set_array([])
        ...
        if trajectories is not None:
            ...
            for i, traj in enumerate(trajectories):
                if traj.shape[1] >= 2:
                    ...
                    ax.plot(traj[:, 0], traj[:, 1], color=line_color, linewidth=10, alpha=1, zorder=1000, solid_capstyle="round")

                    # Plot black circles at each observed point
                    ax.scatter(traj[:, 0], traj[:, 1], s=15, color="black", alpha=0.7, zorder=1002, edgecolors="lightgrey", linewidths=1)
...
def plot_comparison_1x3(
    ...
    if apply_corruption:
        ...
        traj_reshaped = DataCorruptionModel.add_multiplicative_noise(
            traj_reshaped, sigma_distribution="uniform", min_sigma=sigma, max_sigma=sigma
        )
    ...
    all_magnitudes = [np.sqrt(gt_U**2 + gt_V**2), np.sqrt(of_U**2 + of_V**2), np.sqrt(odeon_U**2 + odeon_V**2)]
    ...
    def plot_subplot(
        ...
        ax.streamplot(
            ...
            U,
            V,
            color="#bbbbbb",  # color=mag, cmap='viridis',
            density=0.9,
            linewidth=3.0,
            arrowsize=3.0,
            norm=norm,
        ...
        if corrupted_traj is not None and corrupted_mask is not None:
            ...
            if np.any(mask_indices):
                ax.scatter(
                    corrupted_traj[mask_indices, 0],
                    corrupted_traj[mask_indices, 1],
                    s=120,
                    marker="x",
                    color="black",
                    linewidths=4,
                    alpha=0.8,
                    zorder=1001,
                    label="Corrupted observations",
                )

        # Plot each trajectory
        for i, traj in enumerate(trajs):
            color = trajectory_colors[i % len(trajectory_colors)]
            ax.plot(traj[:, 0], traj[:, 1], color=color, linewidth=8, alpha=1, zorder=1000, solid_capstyle="round")
            ax.scatter(traj[:, 0], traj[:, 1], s=15, color="black", alpha=0.5, zorder=1002, edgecolors="lightgrey", linewidths=1)

            # Add colored box with number at start point
            ax.scatter(traj[0, 0], traj[0, 1], s=1000, marker="s", facecolor=color, edgecolors="black", linewidths=3, zorder=1003)
            ax.text(traj[0, 0], traj[0, 1], f"{i}", fontsize=28, ha="center", va="center", color="white", fontweight="bold", zorder=1004)
    ...
    if rho != 0.0 or sigma != 0.0:  # corrupted
        plot_subplot(
            axes[0],
            X,
            Y,
            ...
            vmin,
            vmax,
            corrupted_traj=corrupted_traj_np,
            corrupted_mask=corrupted_mask_np,
            xlim=shared_xlim,
            ylim=shared_ylim,
        )
    else:  # uncorrupted
        plot_subplot(axes[0], X, Y, gt_U, gt_V, [gt_traj_1, gt_traj_2], "Ground Truth", vmin, vmax, xlim=shared_xlim, ylim=shared_ylim)
    if rho != 0.0 or sigma != 0.0:  # corrupted
        plot_subplot(
            axes[1],
            X,
        ...
        plot_subplot(
            axes[1], X, Y, of_U, of_V, [of_traj_pred_1, of_traj_pred_2], "ODEFormer", vmin, vmax, xlim=shared_xlim, ylim=shared_ylim
        )
    if rho != 0.0 or sigma != 0.0:  # corrupted
        plot_subplot(
            axes[2],
            ...
            odeon_U,
            odeon_V,
            [odeon_traj_pred_1, odeon_traj_pred_2],
            "FIM-ODE",
            vmin,
            vmax,
            corrupted_traj=corrupted_traj_np,
    ...
    if with_colorbar:
        ...
        # label font size
        cbar.set_label("Vector Field Magnitude", fontsize=0.75 * FONT_SIZE, labelpad=20)
...
def plot_comparison_all(
    ...
    save_path: Path,
    odeon_checkpoint_path: Optional[Path] = None,
    sigma_rho_list: List[Tuple[float, float]] = [(0.0, 0.0), (0.5, 0.03), (0.9, 0.05)],
):
    """
