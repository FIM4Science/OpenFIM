    ...
    if trajectories is not None:
        ...
        for i, traj in enumerate(trajectories):
            if traj.shape[1] >= 2:
                ...
                # Mark start point
                ax.scatter(traj[0, 0], traj[0, 1], color=color, s=100, 
                          marker='o', edgecolors='black', linewidths=2, 
                          label=f"Start {i+1}" if len(trajectories) > 1 else "Start", 
                          zorder=6)
                # Mark end point
                ax.scatter(traj[-1, 0], traj[-1, 1], color=color, s=100, 
    ...
    for item in filtered_data:
        ...
        try:
            ...
            if use_substituted and substituted and len(substituted) > 0:
                ...
                vector_field_func = create_vector_field_function(expressions, symbols)
                print(f"  Using original equation with constants substituted")
