...
def fhn_dynamics(t: float, x: Sequence[float]) -> List[float]:
    ...
    """
    x1, x2 = x
    return [3.0 * (x1 - x1**3 / 3.0 + x2), (0.2 - 3.0 * x1 - 0.2 * x2) / 3.0]
...
def make_fim_ode_fn(
    ...
    """
    with torch.no_grad():
        wrapped_D, feature_mask, concept = model.trajectory_encoding(trajectories, times, mask)

    D_in = trajectories.shape[-1]  # original state dimension before any padding

    def fim_fn(t: float, y: Array) -> Array:
        with torch.no_grad():
            loc = torch.tensor(y, dtype=torch.float32, device=trajectories.device).reshape(1, 1, -1)  # [1, 1, D_in]
...
def predict_and_integrate_ode(
    ...
    traj_t = torch.tensor(context_trajs, dtype=torch.float32, device=device).unsqueeze(0)  # [1, n_paths, T, D]
    time_t = (
        torch.tensor(context_times, dtype=torch.float32, device=device).unsqueeze(0).unsqueeze(-1).expand(1, n_paths, T, 1).contiguous()
    )  # [1, n_paths, T, 1]
    ...
    if sections is not None:
        ...
        for sec_name, keys in sections:
            ...
            for key in keys:
                ...
                print(f"  {lbl:<22} {key[1]:>5}  {s['mean']:>8.4f} {s['std']:>8.4f} {s['median']:>8.4f} {s['q05']:>8.4f} {s['q95']:>8.4f}")
                first = False
    else:
        ...
        for k in ctx_sizes:
            s = stats[k]
            print(f"  {k:>5}  {s['mean']:>8.4f} {s['std']:>8.4f} {s['median']:>8.4f} {s['q05']:>8.4f} {s['q95']:>8.4f}")
...
def _draw_trajectories(
    ...
    if sol_xy_full is not None:
        ax.plot(sol_xy_full[:, 0], sol_xy_full[:, 1], color="C3", lw=1.5, label="FIM-ODE", zorder=3)
    if gt_xy is not None:
        ax.plot(gt_xy[:, 0], gt_xy[:, 1], "k--", lw=1.5, label="GT (target)", zorder=2)
    if miss_xy is not None:
        ax.plot(miss_xy[:, 0], miss_xy[:, 1], "rx", ms=5, label="GT (missing region)", zorder=4)
...
def compute_case(
    ...
    fim_grid = predict_vector_field_2d(model, ctx_np, ctx_times, x1_range, x2_range, n_grid, device, mask=ctx_mask)
    print(f"  {label:45s}  ({time.time() - t0:.1f}s)")
    return {"label": label, "ctx_list": ctx_list, "sol_xy": sol.y.T, "fim_grid": fim_grid}
...
def plot_phase_figure(
    ...
    for col, (case, subtitle) in enumerate(zip(cases, subtitles), start=1):
        ...
        _draw_trajectories(ax, ctx_list=case["ctx_list"], sol_xy_full=case["sol_xy"], gt_xy=gt_xy, miss_xy=miss_xy)
...
def plot_ref_evaluation(
    ...
    for dim, row, ylabel in [(0, 0, "$x_1$"), (1, 1, "$x_2$")]:
        # VDP-uniform
        ax = axes[row, 0]
        ax.axvspan(ref_vdp_u["train_ts"][0], ref_vdp_u["train_ts"][-1], alpha=0.07, color="gray", label="training window")
        ...
        # VDP-nonuniform
        ax = axes[row, 1]
        ax.axvspan(ref_vdp_n["train_ts"][0], ref_vdp_n["train_ts"][-1], alpha=0.07, color="gray", label="training window")
...
def find_fixed_points(
    ...
    for x0 in seeds:
        try:
            res = minimize(residual_sq, x0, method="L-BFGS-B", options={"ftol": 1e-30, "gtol": 1e-15, "maxiter": 500})
            sol = res.x
        except Exception:
            continue
...
def mocap_pca_to_50d(
    ...
    traj_50d = pca.inverse_transform(traj_pca.reshape(-1, 5)).reshape(n, T, 50)
...
def _build_arg_parser() -> argparse.ArgumentParser:
    ...
    p.add_argument("--data-dir", type=Path, required=True, help="Path to data/ode/hedge_gp_odes_data/")
    p.add_argument("--output-dir", type=Path, default=Path("results/fim-ode"), help="Directory to save tables and figures")
    p.add_argument("--fmt", choices=["md", "tex"], default="md", help="Output format for tables")
    p.add_argument("--device", default="cpu", help="PyTorch device string")
    p.add_argument("--n-runs", type=int, default=100, help="Number of independent realisations for joint experiments")
    return p
...
if __name__ == "__main__":
    ...
    ctx_trajs_fhn, ctx_times_fhn, ctx_mask_fhn, miss_mask_fhn, tgt_ts_fhn, tgt_ys_fhn = preprocess_ref_fhn(ref_fhn)
