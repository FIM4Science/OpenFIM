...
def predict_vector_field(model: PredictionModel, context_trajs: list[trajectory], locations: np.ndarray) -> np.ndarray:
...
def predict_and_integrate_ode(model: PredictionModel, context_trajs: list[trajectory], y0: np.ndarray, t_eval: np.ndarray) -> trajectory:
    ...
    if plot_type == "1D":
        ...
        def _plot_traj_time_series(ax, traj_list, colors, name_prefix: str) -> None:
            for i, t in enumerate(traj_list):
                ts = np.ravel(t.ts)
                c = colors[i]
                ax.plot(ts, t.xs[:, 0], color=c, linewidth=2, linestyle="-", label=f"{name_prefix} {i + 1} $x_1$", alpha=0.9)
                ax.plot(ts, t.xs[:, 1], color=c, linewidth=2, linestyle="--", label=f"{name_prefix} {i + 1} $x_2$", alpha=0.9)
        ...
        if context_trajs_sparse is not None:
            for i, t in enumerate(context_trajs_sparse):
                ...
                ax.scatter(ts, t.xs[:, 0], c="black", s=30, marker="o", label=label, alpha=0.9, zorder=4)
                ax.scatter(ts, t.xs[:, 1], c="black", s=30, marker="o", label=None, alpha=0.9, zorder=4)
        ...
        if plot_type == "2D":
            quiver = ax.quiver(X, Y, U, V, magnitude, cmap="viridis", scale=None, angles="xy", scale_units="xy", width=0.003, alpha=0.3)
        ...
        if context_trajs is not None:
            ...
            for i, ctx_traj_obj in enumerate(context_trajs):
                ...
                if i == 0:
                    ax.scatter(ctx_np[0, 0], ctx_np[0, 1], c="green", s=60, marker="o", label="Start", zorder=5)
                    ax.scatter(ctx_np[-1, 0], ctx_np[-1, 1], c="red", s=50, marker="s", label="End", zorder=5)
                else:
                    ax.scatter(ctx_np[0, 0], ctx_np[0, 1], c="green", s=60, marker="o", zorder=5)
                    ax.scatter(ctx_np[-1, 0], ctx_np[-1, 1], c="red", s=50, marker="s", zorder=5)
                label = f"Context trajectory {i + 1}"
                ax.plot(ctx_np[:, 0], ctx_np[:, 1], color=colors_context[i], linewidth=2, label=label, alpha=1.0)
        ...
        if predicted_trajs is not None:
            ...
            for i, pred_traj_obj in enumerate(predicted_trajs):
                ...
                ax.plot(pred_np[:, 0], pred_np[:, 1], color=colors_pred[i], linestyle="-", linewidth=2, label=label, alpha=0.8)
                if draw_start_and_end_points:
                    ax.scatter(pred_np[0, 0], pred_np[0, 1], c="green", s=35, marker="o", zorder=5, linewidths=2)
                    ax.scatter(pred_np[-1, 0], pred_np[-1, 1], c="red", s=25, marker="s", zorder=5, linewidths=2)
        ...
        if true_trajs is not None:
            ...
            for i, traj in enumerate(true_trajs):
                ...
                ax.plot(traj.xs[:, 0], traj.xs[:, 1], color=colors_true[i], linestyle="-", linewidth=2, label=label, alpha=0.8)
                if draw_start_and_end_points:
                    ax.scatter(traj.xs[0, 0], traj.xs[0, 1], c="green", s=35, marker="o", zorder=5, linewidths=2)
                    ax.scatter(traj.xs[-1, 0], traj.xs[-1, 1], c="red", s=25, marker="s", zorder=5, linewidths=2)
