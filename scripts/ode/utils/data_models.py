...
class trajectory:
    ...
    def __init__(self, xs: np.ndarray, ts: np.ndarray):
        ...
        if xs.ndim == 4:
            ...
            assert xs.shape[0] == 1 and xs.shape[1] == 1, f"xs with 4 dimensions must have shape (1, 1, T, D), got {xs.shape}"
            xs = xs.squeeze(0).squeeze(0)  # Results in (T, D)
        elif xs.ndim == 3:
            # Shape (1, T, D) - squeeze first dimension
            assert xs.shape[0] == 1, f"xs with 3 dimensions must have shape (1, T, D), got {xs.shape}"
