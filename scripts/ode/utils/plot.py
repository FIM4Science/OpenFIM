...
from utils.helpers import load_odeon_model_from_checkpoint, predict_vector_field


def plot_3d_paths(
    *,
    pred_list: list[trajectory] = [],
    ctx_list: list[trajectory] = [],
    obs_list: list[trajectory] = [],
    ...
    for traj in pred_list:
        ...
        ax.scatter(
            ...
            edgecolors="black",
            linewidths=1,
            label="Start" if traj == pred_list[0] else "",
        )
        ...
        ax.scatter(
            ...
            edgecolors="black",
            linewidths=1,
            label="End" if traj == pred_list[0] else "",
        )
...
def plot_2d_paths(
    *,
    ...
    for traj in pred_list:
        ...
        ax.scatter(
            [end_x], [end_y], color="black", s=20, marker="s", edgecolors="black", linewidths=1, label="End" if traj == pred_list[0] else ""
        )
...
def plot_3d_vector_field(*, model: PredictionModel, context_trajs: list[trajectory] or None = None):
    ...
    print(type(np.meshgrid(np.linspace(-1, 1, 8), np.linspace(-1, 1, 8), np.linspace(-1, 1, 8))))
...
if __name__ == "__main__":
    ...
    context_trajs: list[trajectory] = trajectory_list_from_h5_files(
