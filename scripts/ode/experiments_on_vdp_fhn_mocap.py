...
from ODEs import FHN_ode, VDP_ode
...
from utils.helpers import load_odeon_model_from_checkpoint, plot_vector_field_and_trajectories, predict_and_integrate_ode
...
def vdp_task_1(model: PredictionModel, data_path: Path, plot=False, plot_type: str = "2D"):
    ...
    if plot:
        true_xs_first_half = solve_ivp(VDP_ode, (0.0, 7.0), y0, t_eval=np.linspace(0, 7, 100), method="RK45").y.T  # (100,2)
        ...
        if isinstance(plot, matplotlib.axes.Axes):
            if plot_type == "2D":
                plot_vector_field_and_trajectories(
                    model,
                    context_trajs_sparse=[context_traj],
                    predicted_trajs=[pred_traj_first_half, pred_traj_second_half],
                    true_trajs=[true_traj_first_half, true_traj_second_half],
                    ax=plot,
                )
                ...
                plot_vector_field_and_trajectories(
                    ...
                    true_trajs=[true_traj_first_half, true_traj_second_half],
                    ax=plot,
                    plot_type="2D_streamplot",
                    show_legend=False,
                    draw_start_and_end_points=False,
                )
            elif plot_type == "1D":
    ...
    MSE = (
        (pred_traj_second_half.xs - true_xs_second_half) ** 2
    ).mean()  # You may think this underestimates by factor 2, but that's how the GP-ODE repo does it...
    return MSE
...
def fhn_task(model: PredictionModel, data_path: Path, fine_MSE=False, plot=False, plot_type="2D"):
    ...
    if plot:
        if isinstance(plot, matplotlib.axes.Axes):
            if plot_type == "2D":
                plot_vector_field_and_trajectories(
                    model, context_trajs_sparse=[context_traj], predicted_trajs=[pred_traj], true_trajs=[true_traj], ax=plot, plot_type="2D"
                )
            elif plot_type == "2D_streamplot":
                ...
                plot_vector_field_and_trajectories(
                    model, context_trajs_sparse=[context_traj], predicted_trajs=[pred_traj], true_trajs=[true_traj], ax=plot, plot_type="1D"
                )
        else:
            plot_vector_field_and_trajectories(
                model, context_trajs_sparse=[context_traj], predicted_trajs=[pred_traj], true_trajs=[true_traj]
            )
    ...
    if fine_MSE:
        ...
        MSE = (
            (((pred_traj.xs - true_traj.xs) ** 2) * np.column_stack((quadrant_mask, quadrant_mask))).mean() * 1000 / quadrant_mask.sum()
        )  # It's unclear what this factor should be...
        return MSE
    else:
        ...
        if pkl_path.exists():
            with open(data_path / "experiment_data.pkl", "rb") as f:
                ...
                MSE = (
                    (((pred_xs - true_xs) ** 2) * experiment_data.loss_mask).sum() / 12.0
                )  # It's unclear what this factor should be... I guess judging by the implementation for VDP, I should divide by 12 here
        else:
            ...
            pred_xs = predict_and_integrate_ode(
                model,
                [context_traj],
                y0,
                test_ts.reshape(
            ...
            MSE = (
                ((pred_xs - true_xs) ** 2).sum() / 10.0
            )  # / 12.0       # It's unclear what this factor should be... I guess judging by the implementation for VDP, I should divide by 12 here
...
def mocap_task(model: PredictionModel, data_path: Path, plot=False, dim_left=3, left_only=False, sublist: tuple = None):
    ctx_trajs_5d: list[trajectory] = trajectory_list_from_h5_files(
        path_to_xs=data_path / "obs_values.h5", path_to_ts=data_path / "obs_times.h5"
    )
    ...
    if sublist is None:
        pred_trajs_left = [predict_and_integrate_ode(model, ctx_trajs_left, traj_left.xs[0, :], pred_ts) for traj_left in test_trajs_left]
    else:
        pred_trajs_left = [
            predict_and_integrate_ode(model, [ctx_trajs_left[i] for i in sublist], traj_left.xs[0, :], pred_ts)
            for traj_left in test_trajs_left
        ]
    ...
    if plot:
        ...
        if dim_left == 2:
            plot_vector_field_and_trajectories(model, context_trajs=ctx_trajs_left, predicted_trajs=pred_trajs_left)
    ...
    pred_trajs_5d = [
        trajectory(xs=np.column_stack([pred_traj_left.xs, pred_traj_right.xs]), ts=pred_ts)
...
def mocap_task_new(model: PredictionModel, data_path: Path, plot=False, dim_left=3, left_only=False, sublist: tuple = None):
    ...
    if left_only:
        pred_trajs_5d = [
            trajectory(xs=np.column_stack([left.xs, right.xs]), ts=pred_ts) for left, right in zip(pred_trajs_left, pred_trajs_right)
        ]
    else:
        pred_trajs_5d = [
            trajectory(xs=np.column_stack([left.xs, right.xs[:, 1:]]), ts=pred_ts) for left, right in zip(pred_trajs_left, pred_trajs_right)
        ]
...
def plot_fhn(ax: plt.Axes):
    ...
    ax.plot(true_traj_points[:, 0], true_traj_points[:, 1], "b-", label="True integral curve", linewidth=2, alpha=0.8)
...
if __name__ == "__main__":
    ...
    model = load_odeon_model_from_checkpoint(
        "models/vdp2/vdp2_losssteps=49_hmax=0.025_loss=mse/vdp2_losssteps=49_hmax=0.025_loss=mse_01-16-1128/checkpoints", epoch=None
    )
    print(vdp_task_2(model, Path("experiments/vdp2/data") / "0", plot=True))
    for i in range(100):
        model = load_odeon_model_from_checkpoint(
            "models/vdp2/vdp2_losssteps=49_hmax=0.025_loss=mse/vdp2_losssteps=49_hmax=0.025_loss=mse_01-16-1128/checkpoints", epoch=201 + i
        )
    ...
    model = load_odeon_model_from_checkpoint(
        "models/vdp1/vdp1_losssteps=49_ninter=2_loss=mse/vdp1_losssteps=49_ninter=2_loss=mse_01-15-2324/checkpoints", epoch=None
    )
    ...
    # 1 step:
    model = load_odeon_model_from_checkpoint(
        "models/fhn/fhn_losssteps=3_hmax=0.001_loss=mse/fhn_losssteps=3_hmax=0.001_loss=mse_01-16-1047/checkpoints", epoch=None
    )
    print(fhn_task(model, Path("experiments/fhn/data") / "0", fine_MSE=False, plot=True))
    for i in range(100):
        model = load_odeon_model_from_checkpoint(
            "models/fhn/fhn_losssteps=3_hmax=0.001_loss=mse/fhn_losssteps=3_hmax=0.001_loss=mse_01-16-1047/checkpoints", epoch=201 + i
        )
    ...
    if False:
        model = load_odeon_model_from_checkpoint(
            "models/vdp1/vdp1_losssteps=15_ninter=16_loss=mse/vdp1_losssteps=15_ninter=16_loss=mse_01-15-2012/checkpoints"
        )
    ...
    if False:
        model = load_odeon_model_from_checkpoint(
            "models/vdp1/vdp1_losssteps=15_ninter=16_loss=mse/vdp1_losssteps=15_ninter=16_loss=mse_01-15-2039/checkpoints"
        )
        print(vdp_task_1(model, Path("experiments/vdp1/data") / "2", plot=True))
        for i in range(100):
            model = load_odeon_model_from_checkpoint(
    ...
    quit()
    ckpt_path = Path(
        "models/mocap39short/mocap39short_nsteps=25_ninter=3_loss=mse/mocap39short_nsteps=25_ninter=3_loss=mse_01-14-0409/checkpoints"
    )
    ...
    print(
        "%.2f"
        % mocap_task(
            model, Path("experiments/mocap/") / "mocap39long" / "data/test/5d", plot=True, dim_left=3, left_only=True, sublist=None
        )
    )
