"""
experiments/fim-ode-finetune.py
================================
...
-----
    cd experiments
    python fim-ode-finetune.py --task vdp-u --epochs 200 --lr 3e-5
...
def load_fim_ode_hf(device: str = "cpu") -> FimOdeonUnified:
    """Download and load FIM-ODE from HuggingFace Hub."""
    from huggingface_hub import hf_hub_download

    _REPO_ID   = "FIM4Science/fim-ode"
    _SUBFOLDER = "base_model/checkpoints/best-model"

    config_path  = hf_hub_download(_REPO_ID, f"{_SUBFOLDER}/config.json")
    weights_path = hf_hub_download(_REPO_ID, f"{_SUBFOLDER}/model.safetensors")
...
def load_fim_ode_local(checkpoint_dir: Path, device: str = "cpu") -> FimOdeonUnified:
    """Load FIM-ODE from a local safetensors checkpoint directory."""
...
# =============================================================================
# 3. Core integration helpers (mirroring TrainIntegrator, no config needed)
# =============================================================================
...
class FimOdeFinetuner:
    ...
    Parameters
    ----------
    model          : FimOdeonUnified already on the correct device
    device         : "cpu" | "cuda" | "mps"
    ...
    def finetune(
        ...
        eval_fn: Optional[Callable[[FimOdeonUnified], float]] = None,
        eval_every: int = 20,
        writer=None,
...
# =============================================================================
# 5. Evaluation helpers (integration + MSE)
# =============================================================================
...
def make_vdp_eval_fn(data: Dict, device: str) -> Callable:
    ...
    def eval_fn(model: FimOdeonUnified) -> float:
...
def parse_args():
    ...
    p.add_argument("--data-dir",  default=str(_HERE / "hedge_gp_odes_data"),
                   help="Directory containing vdp_uniform.npz etc.")
    p.add_argument("--ckpt-dir",  default=str(_HERE / "checkpoints" / "finetune"),
                   help="Directory to save checkpoints.")
    p.add_argument("--run-dir",   default=str(_HERE / "runs"),
