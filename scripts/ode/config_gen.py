...
def create_configs(base_config: dict, train_path: Path, valid_path: Path, test_path: Path, task_name: str, max_num_points: int,
                        traj_loss_steps=(10,), intermediate_steps_per_step=(1,), use_h_max=False, h_maxs=(0.01,), loss_types=("mse","l1"),
                        with_noise=False, ic_noise_scales=(0.0,), step_noise_scales=(0.0,), lr=1.e-05):
                ...
                for loss_type in loss_types:
                    for ic_noise_scale in ic_noise_scales:
                        for step_noise_scale in step_noise_scales:
                            assert loss_steps < max_num_points, "Number of loss steps must be strictly less than observed trajectory length!"
                            ...
                            config["model"]["train_config"]["ic_noise_scale"] = ic_noise_scale if with_noise else 0.
                            config["model"]["train_config"]["step_noise_scale"] = step_noise_scale if with_noise else 0.
                            ...
                            config["dataset"]["data_dirs"]["test"] = (str(test_path),) if test_path is not None else tuple()
...
def generate_configs_for_different_noises():
    ...
    all_configs_vp1 = []
    for i in range(10):
        all_configs_vp1 += create_configs(base_config, train_path=data_path/f"{i}", valid_path=data_path/f"{i}", test_path=None, task_name="vdp1", max_num_points=50, **grid_search_vp1)
    ...
    grid_search_vp2 = {
        "use_h_max": True,
        "h_maxs": (.025,),       #(7./50., 7./100., 7./200., 7./400.),
        "traj_loss_steps": (49,),   #(3,6,10,15,25,45),
        "with_noise": False,
        "loss_types": ("mse",)
    }
    ...
    all_configs_vp2 = []
    for i in range(10):
        all_configs_vp2 += create_configs(base_config, train_path=data_path/f"{i}", valid_path=data_path/f"{i}", test_path=None, task_name="vdp2", max_num_points=50, **grid_search_vp2)
    ...
    all_configs_fhn = []
    for i in range(10):
        all_configs_fhn += create_configs(base_config, train_path=data_path/f"{i}", valid_path=data_path/f"{i}", test_path=None, task_name="fhn", max_num_points=19, **grid_search_fhn)
...
def config_gen():
    ...
    all_configs_vp1 = []
    for i in range(10):
        all_configs_vp1 += create_configs(base_config, train_path=data_path/f"{i}", valid_path=data_path, test_path=None, task_name="vdp1", max_num_points=50, **grid_search_vp1)
    for i, config in enumerate(all_configs_vp1):
        configs_save_path = Path(f"experiments/vdp1/configs/config_noise_{i}.yaml")
        configs_save_path.parent.mkdir(parents=True, exist_ok=True)
        save_yaml(config, configs_save_path)
    ...
    grid_search_vp2 = {
        "use_h_max": True,
        "h_maxs": (7./400.), #(7./50., 7./100., 7./200., 7./400.),
        "traj_loss_steps": (15,), #(3,6,10,15,25,45),
        "with_noise": False
    }
    data_path = Path("experiments/vdp2/data/0")
    all_configs_vp2 = create_configs(base_config, train_path=data_path, valid_path=data_path, test_path=None, task_name="vdp2", max_num_points=50, **grid_search_vp2)
    for i, config in enumerate(all_configs_vp2):
    ...
    grid_search_fhn = {
        "use_h_max": True,
        "h_maxs": (.0125,), #(.2, .1, .05, .025, .0125),
        "traj_loss_steps": (11,), # (3,5,7,9,11,14,18),
        "with_noise": False
    }
    data_path = Path("experiments/fhn/data/0")
    all_configs_fhn = create_configs(base_config, train_path=data_path, valid_path=data_path, test_path=None, task_name="fhn", max_num_points=19, **grid_search_fhn)
    for i, config in enumerate(all_configs_fhn):
    ...
    mocap_experiments = {
        ...
        "mocap35short": {
            ...
            "intermediate_steps_per_step": (1, 2, 5),
            "ic_noise_scales": (0.01, 0.05, 0.25),
            "step_noise_scales": (.35,), #(0.1, 0.35, 1.0, 2.0),
            "loss_types": ("mse", "l1")
        },
        "mocap35long": {
            "max_num_points": 250,
    ...
    for (task_name, experiment_dict) in mocap_experiments.items():
        ...
        all_configs = create_configs(base_config, train_path=train_path, valid_path=valid_path, test_path=test_path, task_name=task_name, **experiment_dict)
