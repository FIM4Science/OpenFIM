...
from utils.plot import plot_2d_paths, plot_3d_paths
...
class Data:
    ...
    def to_torch(self, device="cpu"):
        return Data(torch.from_numpy(self.ys).to(device=device), torch.from_numpy(self.ts).to(device=device), container_type="torch")

    def to_numpy(self):
        return Data(self.ys.cpu().numpy(), self.ts.cpu().numpy(), container_type="numpy")
...
class MocapDataset(object):
    def __init__(
        ...
        if data_normalize:
            ...
            xs_train, xs_valid, xs_test = self.data_normalize(xs_train), self.data_normalize(xs_valid), self.data_normalize(xs_test)
        else:
        ...
        if pca_normalize:
            ...
            pca_s = (
                xs_train[:, :].std((0, 1), keepdims=True) + 1e-5
            )  # This is a lot bigger and actually necessary               Shape (1,1,5)
            self.pca_normalize = Normalize(pca_m, pca_s)  # Normalization object for post-PCA
            xs_train, xs_valid, xs_test = self.pca_normalize(xs_train), self.pca_normalize(xs_valid), self.pca_normalize(xs_test)
        else:
...
class MocapDownloader:
    BASE_DATA_URL = (
        "https://raw.githubusercontent.com/hegdepashupati/gaussian-process-odes/c084729817e09cb3910b45ec268eb4688e0a44f8/data/mocap"
    )
...
class FimMocapDatasetBuilder:
    ...
    def build_train_dataset_for(self, subject: Literal["09", "35", "39"], seqlen: int):
        ...
        data_pca = MocapDataset(
            data_path=str(self.MOCAP_DATA_PATH),
            subject=subject,
        ...
        self._save_dataset(data_pca, save_path=self.MOCAP_DATA_PATH / subject / f"seqlen{seqlen}", save_dataset_object_additionally=True)
...
def data_gen_mocap():
    ...
    for experiment_name, sequence_length in experiments.items():
        ...
        # training data
        FimMocapDatasetBuilder()._save_training_data(
            dataset, save_path=Path(f"experiments/mocap/mocap{subject}{long_short}/data/train/5d"), save_dataset_object_additionally=True
        )

        # testing data
        FimMocapDatasetBuilder()._save_testing_data(
            dataset, save_path=Path(f"experiments/mocap/mocap{subject}{long_short}/data/test/5d"), save_dataset_object_additionally=True
        )

        # validation data
        FimMocapDatasetBuilder()._save_validation_data(
            dataset, save_path=Path(f"experiments/mocap/mocap{subject}{long_short}/data/valid/5d"), save_dataset_object_additionally=True
        )
...
if __name__ == "__main__":
    ...
    FimMocapDatasetBuilder()._save_dataset(
        dataset, save_path=Path(f"experiments/mocap/subject_{subj}/seqlen_{seqlen}"), save_dataset_object_additionally=True
    )
    ...
    for i in range(x.shape[0]):
        ax.plot(x[i, :, 0], x[i, :, 1], x[i, :, 2], color=colors[i], linewidth=2, alpha=0.7, label=f"Trajectory {i + 1}")
        # Mark start point
        ax.scatter(x[i, 0, 0], x[i, 0, 1], x[i, 0, 2], color=colors[i], s=100, marker="o", edgecolors="black")
