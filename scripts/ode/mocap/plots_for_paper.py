...
def _vector_field_2d(model, traj_t, time_t, mask_t,
                     di, dj, xlim, ylim, trn_ys, device,
...
def plot_mocap_subject(
    ...
    for col, (di, dj) in enumerate(_PROJ_PAIRS):
        ...
        xi_g, xj_g, U, V = _vector_field_2d(
            model, traj_t, time_t, mask_t,
            di, dj, xlim, ylim, trn_ys, device, grid_resolution,
        )
        ax.streamplot(xi_g, xj_g, U, V,
                      color="#bbbbbb", density=0.9, linewidth=3., arrowsize=3.)

        # ── GT: black "x" crosses ───────────────────────────────────────────
        for k in range(n_test):
            ax.scatter(
                tst_ys[k, :, di], tst_ys[k, :, dj],
                s=80, marker="x", color="black", linewidths=2, alpha=0.8,
                zorder=1002,
            )
        ...
        for k in range(n_test):
            pr = preds[k]
            ax.plot(pr[:, di], pr[:, dj],
                    color=COLOR_PRED, linewidth=8, alpha=1.0,
                    zorder=1000, solid_capstyle="round")
            ax.scatter(pr[:, di], pr[:, dj],
                       s=15, color="black", alpha=0.5,
                       zorder=1002, edgecolors="lightgrey", linewidths=1)
            ax.scatter(pr[0, di], pr[0, dj],
                       s=1000, marker="s",
                       facecolor=COLOR_PRED, edgecolors="black", linewidths=3,
                       zorder=1003)
            ax.text(pr[0, di], pr[0, dj], str(k),
                    fontsize=28, ha="center", va="center",
                    color="white", fontweight="bold", zorder=1004)
        ...
        ax.tick_params(axis="both", which="major", labelsize=0.75 * FONT_SIZE,
                       width=2, length=6)
        ax.set_xlim(xlim)
        ax.set_ylim(ylim)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--ckpt-dir", default=None,
                        help="Local checkpoint dir (uses HuggingFace pretrained if omitted)")
