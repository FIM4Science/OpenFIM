...
def save_h5_files(dataset: MocapDataset, out_dir: Path) -> None:
    ...
    obs_times = dataset.trn.ts[np.newaxis, np.newaxis, :, np.newaxis].repeat(n_traj, axis=1)  # (1, n_traj, seqlen, 1)
    obs_mask = np.ones((*obs_values.shape[:-1], 1), dtype=bool)  # (1, n_traj, seqlen, 1)
