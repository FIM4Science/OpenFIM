...
def evaluate_model(model_path: Path, data: dict):
    """
    Return estimated drift and diffusion of model at locations.

    Args:
        model_path (Path): Path to checkpoint dir.
        data (dict): includes keys obs_times, obs_values, locations

    Returns:
        results (dict): keys: drift_at_locations, diffusion_at_locations
    """
    device = "cuda" if torch.cuda.is_available() else "cpu"

    model = FIMSDE.load_model(model_path).to(device).eval()

    data = {k: v for k, v in data.items() if k in ["obs_times", "obs_values", "locations"]}
    D = data["obs_values"].shape[-1]

    # pad data to max dim
    data["obs_values"] = torch.concat([data["obs_values"], torch.zeros(data["obs_values"].shape[:-1] + (3 - D,))], dim=-1)
    data["locations"] = torch.concat([data["locations"], torch.zeros(data["locations"].shape[:-1] + (3 - D,))], dim=-1)

    data = optree.tree_map(lambda x: x.to(device) if isinstance(x, torch.Tensor) else x, data, namespace="fimsde")
    concepts = model(data, training=False, return_losses=False)

    return {
        "drift_at_locations": concepts.drift.detach().to("cpu")[..., :D],
        "diffusion_at_locations": concepts.diffusion.detach().to("cpu")[..., :D],
    }


def get_bisde_5000_dense_points_double_well(locs):
    return {
        "drift_at_locations": -2.0630 + 1.2557 * locs**2 - 1.6117 * locs**3,
        "diffusion_at_locations": np.sqrt(4.0844 * 1 - 1.2654 * locs**2),  # they estimate value under the sqrt
    }


def get_bisde_5000_dense_points_two_d(locs):
    x0, x1 = locs[..., 0], locs[..., 1]

    drift0 = -0.9641 * x1 - 0.9331 * x0**3 - 0.6279 * x1**3
    drift1 = 2.0778 * x0 - 0.2839 * x0**2 + 0.4449 * x0 * x1 - 2.0097 * x0**2 * x1 - 0.5430 * x1**3

    diff0 = 0.9377 + 0.3580 * x1**2 + 0.0826 * np.exp(x0) * x1
    diff1 = 1.0551 + 0.3645 * x0**2

    return {
        "drift_at_locations": np.stack([drift0, drift1], axis=-1),
        "diffusion_at_locations": np.stack([diff0, diff1], axis=-1),
    }
