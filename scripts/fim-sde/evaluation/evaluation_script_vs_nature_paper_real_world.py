...
class ModelResults:
    ...
    def __call__(self, experiment_id: int, dataset_name: str):
        ...
        if self.model_name == "GroundTruth":
            return torch.tensor(self.data[dataset_idx]["real_paths"][experiment_id])[:max_num_paths] # [num_paths, num_time_points, num_dims]
        else:
...
def run_benchmark(benchmark: Benchmark, evaluation_dir):
    ...
    for experiment_id in range(1):
        with open(evaluation_dir / f"results_{experiment_id}.txt","w") as f: 
            for dataset_name in ['wind', 'oil']:
                table = []
                for model_name in ['FIM', 'BISDE']:
                    ...
                    ground_truth_paths = ground_truth_paths.reshape(max_num_paths,-1).unsqueeze(2)
                    model_paths = model_paths.reshape(max_num_paths,-1).unsqueeze(2)
...
if __name__ == "__main__":
    ...
    evaluation_dir: Path = Path(f"evaluations/ksig/{time}/real_world_oil_wind")
    evaluation_dir.mkdir(parents=True, exist_ok=True)
    
    benchmarks = [
        Benchmark(
            GroundTruth = ModelResults("GroundTruth", Path("/cephfs_projects/foundation_models/data/SDE/evaluation/20250130_bisde_real_world_oil_wind/ksig_reference_paths.json")),
            FIM = ModelResults("FIM", Path("/cephfs_projects/foundation_models/data/SDE/evaluation/20250130_bisde_real_world_oil_wind/model_paths.json")),
            BISDE = ModelResults("BISDE", Path("data/processed/test/bisde_empirical_experiments.json")),
        ),
    ]
