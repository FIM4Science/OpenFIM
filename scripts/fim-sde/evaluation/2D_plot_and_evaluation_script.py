...
def run_benchmark_and_plot(benchmark: Benchmark):
    ground_truth_locations, ground_truth_drift, ground_truth_diffusion = (
        benchmark.GroundTruth.locations,
        benchmark.GroundTruth.drift,
        benchmark.GroundTruth.diffusion,
    )
    ...
    if benchmark.ComparisonModel is not None:
        comparison_model_locations, comparison_model_drift, comparison_model_diffusion = (
            benchmark.ComparisonModel.locations,
            benchmark.ComparisonModel.drift,
            benchmark.ComparisonModel.diffusion,
        )
    else:
        comparison_model_locations, comparison_model_drift, comparison_model_diffusion = None, None, None
    assert model_locations.shape == ground_truth_locations.shape, (
        f"Locations have different shapes between model and ground truth data: {model_locations.shape} vs {ground_truth_locations.shape}"
    )
    if comparison_model_locations is not None:
        assert model_locations.shape == comparison_model_locations.shape, (
            f"Locations have different shapes between model and comparison model data: {model_locations.shape} vs {comparison_model_locations.shape}"
        )
    ...
    if comparison_model_locations is not None:
        assert torch.allclose(model_locations, comparison_model_locations), (
            "Locations are not the same between model and comparison model data"
        )

    stride = benchmark.stride
    model_locations_plot, model_drift_plot, model_diffusion_plot = (
        model_locations[:, ::stride],
        model_drift[:, ::stride],
        model_diffusion[:, ::stride],
    )
    ground_truth_drift_plot, ground_truth_diffusion_plot = ground_truth_drift[:, ::stride], ground_truth_diffusion[:, ::stride]
    if comparison_model_locations is not None:
        comparison_model_locations_plot, comparison_model_drift_plot, comparison_model_diffusion_plot = (
            comparison_model_locations[:, ::stride],
            comparison_model_drift[:, ::stride],
            comparison_model_diffusion[:, ::stride],
        )
    else:
        comparison_model_locations_plot, comparison_model_drift_plot, comparison_model_diffusion_plot = None, None, None  # noqa: F841

    table = [["FIM", mse(model_drift, ground_truth_drift), mse(model_diffusion, ground_truth_diffusion)]]
    if comparison_model_locations is not None:
        table.append(
            ["Comparison Model", mse(comparison_model_drift, ground_truth_drift), mse(comparison_model_diffusion, ground_truth_diffusion)]
        )
    ...
    benchmarks = [
        Benchmark(
            ...
            GroundTruth=Dataset(
                "Ground Truth",
                Path(
                    "/cephfs_projects/foundation_models/data/SDE/test/20241223_opper_and_wang_cut_to_128_lenght_paths/two_d_wang_80000_points"
                ),
            ),
            FIM=Dataset(
                "FIM",
                Path(
                    "evaluations/synthetic_datasets/01221033_30k_deg_3_ablation_studies_only_learn_scale/model_evaluations/11M_params/wang_two_d_80000_points/default11M_params_wang_two_d_80000_points.pickle"
                ),
            ),
            ComparisonModel=Dataset(
                "BISDE", Path("data/processed/test/20250117_wang_estimated_equations/bisde_est_2D_synth_80000_points_split_128_length")
            ),
            stride=10,
            zoom_area_drift=dict(xlim=(2, 2.8), ylim=(-1.1, -0.9)),  # noqa: C408
            zoom_area_diffusion=dict(xlim=(1.2, 2.2), ylim=(-2, -1)),  # noqa: C408
        ...
        Benchmark(
            ...
            GroundTruth=Dataset(
                "Ground Truth",
                Path(
                    "/cephfs_projects/foundation_models/data/SDE/test/20241222_svise_1_perc_diffusion_no_additive_noise/damped_cubic_oscillator"
                ),
            ),
            FIM=Dataset(
                "FIM",
                Path(
                    "evaluations/synthetic_datasets/01221033_30k_deg_3_ablation_studies_only_learn_scale/model_evaluations/11M_params/svise_damped_cubic/default11M_params_svise_damped_cubic.pickle"
                ),
            ),
            stride=10,
        ),
        Benchmark(
            ...
            GroundTruth=Dataset(
                "Ground Truth",
                Path(
                    "/cephfs_projects/foundation_models/data/SDE/test/20241222_svise_1_perc_diffusion_no_additive_noise/damped_linear_oscillator"
                ),
            ),
            FIM=Dataset(
                "FIM",
                Path(
                    "evaluations/synthetic_datasets/01221033_30k_deg_3_ablation_studies_only_learn_scale/model_evaluations/11M_params/svise_damped_linear/default11M_params_svise_damped_linear.pickle"
                ),
            ),
            stride=10,
        ),
        Benchmark(
            ...
            GroundTruth=Dataset(
                "Ground Truth",
                Path(
                    "/cephfs_projects/foundation_models/data/SDE/test/20241222_svise_1_perc_diffusion_no_additive_noise/duffing_oscillator"
                ),
            ),
            FIM=Dataset(
                "FIM",
                Path(
                    "evaluations/synthetic_datasets/01221033_30k_deg_3_ablation_studies_only_learn_scale/model_evaluations/11M_params/svise_duffing/default11M_params_svise_duffing.pickle"
                ),
            ),
            stride=10,
        ),
        Benchmark(
            ...
            GroundTruth=Dataset(
                "Ground Truth",
                Path("/cephfs_projects/foundation_models/data/SDE/test/20241222_svise_1_perc_diffusion_no_additive_noise/hopf_bifurcation"),
            ),
            FIM=Dataset(
                "FIM",
                Path(
                    "evaluations/synthetic_datasets/01221033_30k_deg_3_ablation_studies_only_learn_scale/model_evaluations/11M_params/svise_hopf_bifurcation/default11M_params_svise_hopf_bifurcation.pickle"
                ),
            ),
            stride=10,
        ),
        Benchmark(
            ...
            GroundTruth=Dataset(
                "Ground Truth",
                Path(
                    "/cephfs_projects/foundation_models/data/SDE/test/20241222_svise_1_perc_diffusion_no_additive_noise/selkov_glycolysis"
                ),
            ),
            FIM=Dataset(
                "FIM",
                Path(
                    "evaluations/synthetic_datasets/01221033_30k_deg_3_ablation_studies_only_learn_scale/model_evaluations/11M_params/svise_selkov_glycolysis/default11M_params_svise_selkov_glycolysis.pickle"
                ),
            ),
            stride=10,
        ),
