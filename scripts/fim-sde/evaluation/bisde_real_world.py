...
def evaluate_model(model: AModel, dataset: dict, samples_per_initial_state: int, process_log: bool, device: Optional[str] = None):
    ...
    # initial_time = dataset["obs_times"][:, :, 0]  # [1, 1, 1]
    # end_time = dataset["obs_times"][:, :, -1]  # [1, 1, 1]
    # grid_size = dataset["obs_times"].shape[-2]
...
if __name__ == "__main__":
    # ------------------------------------ General Setup ------------------------------------------------------------------------------ #
    dataset_descr = "bisde_real_world"

    # How to name experiments
    experiment_descr = "extracting_data_for_inference_and_ksig"

    model_dicts, models_display_ids = get_model_dicts_600k_deg_3_drift_deg_2_diff()

    results_to_load: list[str] = [
        # "/home/seifner/repos/FIM/evaluations/bisde_real_world/01301850_extracting_data_for_inference_and_ksig/model_evaluations",
    ]
    ...
    # Get datasets
    datasets = {
        # "fb": get_wang_real_world_data(base_data_dir / data_fb_subdir, load_fluctuations=False),
        # "tsla": get_wang_real_world_data(base_data_dir / data_tsla_subdir, load_fluctuations=False),
